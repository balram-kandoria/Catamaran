...
class RudderController(Node):
    def __init__(self):
        ...
        self.pidcontroller = pid.Controller(dt = 0.0001)
    ...
    def rudder_control(self):
        if self.rotation_data is not None and self.imu_data is not None:
            ...
            self.cmd,self.integral,self.error,self.feedback = self.pidcontroller.PID(Kp=0.1,Ki=0.001,Kd=0.01,target=0,integral=self.integral,error_prev=self.error,feedback_prev=self.feedback,feedback=sensor_feedback)
            
            rudderCMD = Float64()
            if abs(self.cmd) > 1.2:
                self.cmd = 1.2 * (self.cmd / abs(self.cmd))
                self.props.publish_data(20.0)
            
            rudderCMD.data = -self.cmd

            self.right_rudder_pub.publish(rudderCMD)
            self.left_rudder_pub.publish(rudderCMD)
            print(rudderCMD.data)
